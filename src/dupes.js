...
function getFiles(options, callback) {
  ...
  walker.on('end', function() {
    var scanDuration = utils.hrtimeToSeconds(process.hrtime(start))
    results.set('scanDuration', scanDuration)
    if (options.printStats) {
      console.log('Scan finished in ' + scanDuration.toFixed(3) + ' seconds.')
    }
...
function processFiles(files, options) {
  ...
  methodsList.forEach(function(method) {
    ...
    step.duration = utils.hrtimeToSeconds(process.hrtime(step.before.time))
    if (options.printStats) {
      console.log('Method finished in ' + step.duration.toFixed(3) + ' seconds.')
    }
  ...
  } else if (options.printStats) {
    console.log('Duplicates file not printed')
  }
  if (options.printStats) {
...
function getSetsStatsObj(sets) {
  return { files: sets.map(function(set) { return set.length }).reduce(utils.sum, 0), pools: sets.length, time: process.hrtime() }
}
