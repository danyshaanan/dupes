...
function resolveDirectory(dir) {
  ...
  if (!(fs.existsSync(fullPath) && fs.statSync(fullPath).isDirectory())) {
    logger.log(clc.red(fullPath + ' is not a directory!'));
    process.exit();
  }
  return fullPath;
}
...
function processFiles(files) {
  var sets = methodsList.reduce(function(sets, method) {
    logger.log(poolsStats(sets) + 'Differentiating files by method:' + method.name + '...');
    return method(sets).filter(function(set) { return set.length > 1; });
  }, [files]);
  logger.log(poolsStats(sets));
  logger.log(resultOutput(sets));
}

function poolsStats(sets) {
  // var totalSize = sets.map(function(set) { return set.map(function(item) { return item.size; }).reduce(sum, 0); }).reduce(sum, 0);
  var poolsCount = sets.length;
  var filesCount = sets.map(function(set) { return set.length; }).reduce(sum, 0);
